...
function propagate_layer_batch(prop_method::IBP, layer::typeof(relu), bound::IBPBound, batch_info)
    return IBPBound(max.(bound.batch_low, 0), max.(bound.batch_up, 0))
end  

function propagate_layer(prop_method::RotatedBox, layer::typeof(relu), reach::RotatedHyperrectangle, batch_info)
    v_list = vertices_list(reach)
    if zero(v_list[1]) ∈ reach
        push!(v_list, zero(v_list[1]))
    end
    M = reach.M
    # @show M
    # @show v_list
    # @show [max.(v, 0) for v in v_list]

    # @show [M' * v for v in v_list]
    u_list = [M' * max.(v, 0) for v in v_list]
    u_mat = hcat(u_list...)
    # @show u_list
    # @show [M * u for u in u_list]
    l = minimum(u_mat, dims=2)
    u = maximum(u_mat, dims=2)
    # @show l
    # @show u
    H = Hyperrectangle(low=l[:], high=u[:])
    rb = RotatedHyperrectangle(M, H)
    # @show vertices_list(rb)
    return rb
end  
...
#     end
#     M = reach.M
#     u_list = [max.(v, 0) for v in v_list]
#     u_mat = hcat(u_list...)
#     l = minimum(u_mat, dims=2)
#     u = maximum(u_mat, dims=2)
#     H = Hyperrectangle(low=l[:], high=u[:])
...
function fast_remove_redundant_generators(Z::Zonotope)
    G = genmat(Z)
    # @show size(G, 2)
    G = LazySets.remove_zero_columns(G)
    cnt_sum = 0
    cnt_ele = 0
    # for i in 1:size(G,2)
    #     gen = G[:,i]
    #     cnt_sum += sum(abs.(gen)) .> 1e-9
    #     cnt_ele += (sum(abs.(gen) .> 1e-9)) > 0
    #     if sum(abs.(gen) .> 1e-9) == 0
    #         @show sum(abs.(gen))
    #     end
    # end
    @show cnt_sum
    @show cnt_ele
    n_gen = size(G, 2)
    @show "removed 0", n_gen
    deleted = false
    done = falses(n_gen)
    hash_bank = Dict()
    effect_round(x) = abs(x) < 1e-6 ? 0 : round(x, sigdigits=3)
    @inbounds for i in 1:n_gen
        gen_v = G[:, i]
        normalized_gen = (gen_v .- mean(gen_v)) ./ maximum(abs.(gen_v))
        h_pos = hash(effect_round.(normalized_gen))
        h_neg = hash(effect_round.(.-normalized_gen))

        function find_value(key, value)
            if !haskey(hash_bank, key) 
                return false
            end
        ...
        (find_value(h_pos, gen_v) || find_value(h_neg, gen_v)) && continue
