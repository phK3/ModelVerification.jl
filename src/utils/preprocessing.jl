...
function onnx_node_to_flux_layer(vertex)
    ...
    elseif node_type == NaiveNASlib.var"#342#344"{typeof(-)}
        return -
    elseif occursin("relu", string(node_type))
        return NNlib.relu
    elseif node_type == NaiveNASlib.var"#335#336"{Int64} # concat
        if vertex.base.base.computation.dims == 1
            return vcat
        elseif vertex.base.base.computation.dims == 2
            return hcat
        else
            return x -> cat(x..., dims=vertex.base.base.computation.dims)
        end
    else
        # @show vertex
...
function remove_layer_act(l)
    ...
    elseif l isa BatchNorm
        ...
        @warn "Decoupling activation for $l is not implemented. The inference output may be incorrect. Verification may be affected."
    end
end
